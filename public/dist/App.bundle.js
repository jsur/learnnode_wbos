...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; //skip this from running if no input
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    //if up down enter not pressed, doesnt matter
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
          ...
          placeLat = _place$location$coord[1]; //Destructuring again here!
    ...
    //then zoom the map for perfect fit
...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // a form element with the name "heart"
